...
class Main:
    ...
    def __init__(self):
        ...
        self.cb_theme = CTkRadioButton(self.frame_settings,command=self.change_theme,variable= self.var_theme,text="Темная тема",value=1)#, variable=self.var_theme)
        self.cb_theme1 = CTkRadioButton(self.frame_settings,command=self.change_theme,variable= self.var_theme,text="Светлая тема",value=0)#, variable=self.var_theme)
    ...
    def print_timer1(self):
        ...
        if self.is_start_timer:
            if not self.is_run_timer:            
               ...
               if self.step !=0:
                   self.time_sum.append(time.time()-self.time_now-self.sleep_time)
    ...
    def char_generate(self,n1,n2):
        ...
        if self.age_user >=13:
            ...
            if  char_t == "-" and n1<n2:
                ...
                raise Exception("CharGeneratorError ----> name example is not defaint")
        ...
        if 0 < self.step < self.num_examples and self.text_result.get().isdigit(): #We must save answer and show new math example
           # mb.showinfo("if",self.text_result.get().isdigit())
            ...
            self.all_results[self.example] =[self.step,
                                             ...
                                             self.result,
                                             self.text_result.get()==self.result] #step ; answer ; real result; True or False answer
            ...
            self.label_hard_cft.configure(text="")#self.label_hard_cft.configure(text=f"Сложность: {self.hard_cft+1}/3") if self.age_user > 13 else self.label_hard_cft.configure(text="")
            ...
            self.all_results[self.example] =[self.step,
                                             self.text_result.get(),
                                             self.result,
                                             self.text_result.get()==self.result]
            self.end_time = time.time() # Stop timer
            self.rank = Rank.start(self.num_examples,round(self.end_time-self.start_time-self.num_examples*self.sleep_time,1),self.true_or_false,round(sum(self.time_sum)) if self.age_user > 13 else 1,sum(self.hard_list) if self.age_user > 13 else 1)
         ...
         #   mb.showinfo("elif2",self.text_result.get().isdigit())
            ...
            if self.age_user <=9:
                    ...
                    self.example = self.char_generate(n1,n2)                   
            self.hard_list.append(self.hard_cft)
            self.now = self.example       
            self.math_example.configure(text=self.example)
            self.result = eval(self.example)
            self.label_hard_cft.configure(text="")# self.label_hard_cft.configure(text=f"Сложность: {self.hard_cft+1}/3") if self.age_user > 13 else self.label_hard_cft.configure(text='')
