...
class Main:
    ...
    def show_results(self):
        ...
        for i in self.all_results:
            ...
            CTkLabel(self.window,text=f"{self.all_results[i][0]}. {i} ваш ответ: {self.all_results[i][1]}; Правильный ответ: {self.all_results[i][2]}",text_color="green" if self.all_results[i][3] else "red",font=("Bold",30)).pack(anchor=W)
        CTkLabel(self.window,text=f"Общее время: {round(self.end_time-self.start_time-self.num_examples*self.sleep_time,1)}        Среднее время на один пример: {round(round(self.end_time-self.start_time-self.num_examples*self.sleep_time,1)/self.num_examples,1)}",text_color="white",font=("Bold",40)).pack(anchor=CENTER)
    def next(self):
        global all_results
        self.entry_result.focus()
